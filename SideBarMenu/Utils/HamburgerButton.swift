...
//  SideBarMenu
//
//  Created by Yonadab on 11/29/18.
...
class HamburgerButton : UIButton {
    ...
    let outline: CGPath = {
        ...
        path.move(to: CGPoint(x: 11, y: 27 ))
...
//            layer?.strokeColor = UIColor.blue.cgColor
            layer?.strokeColor = #colorLiteral(red: 0, green: 0.4784313725, blue: 1, alpha: 1)
            layer?.lineWidth = 2.5
            layer?.miterLimit = 2
            layer?.lineCap = CAShapeLayerLineCap.round
            layer?.masksToBounds = true
            
            let strokingPath = CGPath(__byStroking: (layer?.path!)!, transform: nil, lineWidth: 2.5, lineCap: .round, lineJoin: .miter, miterLimit: 2)
